...
public class HttpMethods3 {
    ...
    public static final String BASE_URL = "http://192.168.0.2:8001/"; // 测试-新
     ...
     * 设置支付密码
     */
    public void setPayPassWord(Subscriber subscriber, String oldPassword, String newPassword) {
        HashMap<String, Object> map = new HashMap<String, Object>();
        map.put("oldPassword", oldPassword);
        map.put("newPassword", newPassword);
        map.put("confirmPassword", newPassword);
        Observable observable = movieService.getlistgoodsInfoM("Bearer " + MyApplication.getInstances().getToken(), map)
    ...
    public void AddAddressRequest(Subscriber<HttpResult3> subscriber, AddressBean addressBean) {
        Observable observable = movieService.getlistgoodsInfo("Bearer " + MyApplication.getInstances().getToken(), addressBean).map(new HttpMethods3.HttpResultFunc2<AddressBean>());
        toSubscribe(observable, subscriber);
    }
    ...
    public void AddressList(Subscriber<HttpResult3> subscriber,int page, int size) {
        ...
        Observable observable = movieService.getAddressList("Bearer " + MyApplication.getInstances().getToken(),map).map(new HttpMethods3.HttpResultFunc2<AddressBean>());
        toSubscribe(observable, subscriber);
    }
    ...
    private class HttpResultFunc2<T> implements Func1<HttpResult3<T>, T> {
        ...
        public T call(HttpResult3<T> httpResult) {
            if (!httpResult.isResult()) {
                throw new ApiException3(httpResult.getErrCode(), httpResult.getMessage());
            }
